...
There are 1846 rows and 3 variables. The **dataset** is **x**, and **Y**

Let's take a look at what these datasets are.
To do so we can make a *frequency table* of the dataset variable:

```{r plot-dino, fig.height=3, fig.width=6, warning=TRUE, paged.print=TRUE}
datasaurus_dozen %>%
  count(dataset) %>%
  print(13)
```
...
The correlation coefficent for star is **-0.062961**. This dataset is of higher value than dino's at **-0.06447185** 
...
The correlation coefficent is **-0.06834336	** and is of lower value than star at **-0.062961**
